...
def api_places_nearby():
    ...
    # practices that happen to also be tagged 'hospital'. We require the place's
    # primaryType to be 'hospital' AND the place to NOT be primarily a doctor's
    # surgery, physio clinic, dentist, etc.
    if place_type == 'hospital':
        ...
        for p in places:
            ...
            # Keep only if primaryType is hospital and excluded types are not present
            if primary == 'hospital' and not (types & EXCLUDE_AS_HOSPITAL):
                filtered.append(p)
        # If filtering removed everything, fall back to: primaryType == hospital,
        # ignoring exclusion (better than nothing).
        if not filtered:
            filtered = [p for p in places if p.get('primaryType') == 'hospital']
        # If STILL nothing, return null rather than a misleading medical centre
        if not filtered:
            return jsonify({'name': None, 'km': None})
        places = filtered
