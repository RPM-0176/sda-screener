...
DASHBOARD_CONTENT = open(DASHBOARD_HTML).read() if os.path.exists(DASHBOARD_HTML) else ''
...
def dashboard():
    ...
    # Try file first, fall back to embedded
    if os.path.exists(DASHBOARD_HTML):
        return send_file(DASHBOARD_HTML)
    return make_response(DASHBOARD_CONTENT)
...
def get_shortlist():
    ...
    db = get_db()
    row = db.execute('SELECT property_ids FROM shortlists WHERE user_id=?', (uid,)).fetchone()
    db.close()
    ids = json.loads(row['property_ids']) if row else []
...
def save_shortlist():
    ...
    db = get_db()
    db.execute('''INSERT INTO shortlists (user_id, property_ids) VALUES (?,?)
                  ON CONFLICT(user_id) DO UPDATE SET property_ids=excluded.property_ids, updated_at=datetime('now')''',
               (uid, ids))
    db.commit()
    db.close()
...
def upload_csv():
    ...
    try:
        ...
        db = get_db()
        db.execute('''INSERT INTO property_data (state, csv_data, row_count, uploaded_by)
                      VALUES (?,?,?,?)
                      ON CONFLICT(state) DO UPDATE SET
                      csv_data=excluded.csv_data, row_count=excluded.row_count,
                      uploaded_by=excluded.uploaded_by, uploaded_at=datetime('now')''',
                   (state, csv_data, rows, request.current_user['username']))
        db.commit()
        db.close()
        return redirect('/admin/upload_page?msg='+state.upper()+'+uploaded+successfully+('+str(rows)+'+properties)&msg_type=ok')
    except Exception as e:
        return redirect('/admin/upload_page?msg=Upload+failed:+'+str(e)[:50]+'&msg_type=er')
