...
def api_places_nearby():
    ...
    # For 'hospital' searches, we need to be strict because Google tags many
    # things as 'hospital' that aren't (specialists, day clinics, individual doctors,
    # even non-medical organisations with 'hospital' in their company name).
    # The most reliable filter: require the place name to contain a hospital-system
    # keyword. Real hospitals almost always identify themselves this way:
    #   - "Royal Melbourne Hospital"
    #   - "Austin Hospital"
    #   - "Northern Health"
    #   - "Mercy Hospital for Women"
    # while bad results almost never do (e.g. "Anastasia Hueston", "ABC Specialists").
    if place_type == 'hospital':
        ...
        # Allow-list: at least one of these keywords must appear in the name
        # (case-insensitive, substring check). Real public hospitals almost always
        # identify themselves with one of these. Note: "medical centre" is NOT
        # included because it overwhelmingly refers to GP clinics, not hospitals.
        REQUIRED_NAME_KEYWORDS = [
            'hospital', 'health service', 'health network'
        ]
        # Exclude these even if name matches above (e.g. "Animal Hospital",
        # "Hospital Engineering Institute").
        NAME_EXCLUDE_KEYWORDS = [
            'private', 'day hospital', 'day surgery', 'cosmetic',
            'plastic surgery', 'ivf', 'eye hospital',
        ...
        for p in places:
            ...
            if (primary == 'hospital'
                    and not (types & EXCLUDE_AS_HOSPITAL)
                    and name_required(nm)
                    and not name_excluded(nm)):
                filtered.append(p)
        # Fallback: drop the type-restriction (allow primaryType != hospital but with
        # a hospital keyword in the name) — catches places where Google tags a
        # legitimate hospital with a different primaryType.
        if not filtered:
            filtered = [p for p in places
                        if name_required(((p.get('displayName') or {}).get('text')) or '')
                        and not name_excluded(((p.get('displayName') or {}).get('text')) or '')]
        if not filtered:
            return jsonify({'name': None, 'km': None})
        places = filtered
