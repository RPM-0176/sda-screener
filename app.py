...
def api_places_nearby():
    ...
    # For 'hospital' searches, filter out medical centres / GP clinics / specialist
    # practices that happen to also be tagged 'hospital'. Also filter by name to
    # exclude private day-surgery and cosmetic clinics.
    if place_type == 'hospital':
        ...
        # Case-insensitive name keyword exclusions — tokens that strongly suggest
        # this is NOT a public/general hospital. Includes both private clinics and
        # non-hospital organisations that just happen to have 'hospital' in their name
        # (institutes, training bodies, foundations, etc.).
        NAME_EXCLUDE_KEYWORDS = [
            'private', 'day hospital', 'day surgery', 'cosmetic',
            'specialist', 'specialists', 'plastic surgery', 'dental',
            'cardiology', 'fertility', 'ivf', 'eye hospital', 'eye clinic',
            'physio', 'rehab clinic',
            # Non-hospital orgs wrongly tagged
            'institute', 'foundation', 'association', 'society',
            'training', 'college', 'university',
            'engineering', 'consulting', 'consultancy',
            'pty ltd', 'pty.', 'limited', 'group ',
        ...
        for p in places:
            ...
            if (primary == 'hospital'
                ...
                filtered.append(p)
        # Fallback 1: drop name-based exclusion (still require primaryType=hospital, no excluded types)
        if not filtered:
            filtered = [p for p in places
                        if p.get('primaryType') == 'hospital'
                        and not (set(p.get('types') or []) & EXCLUDE_AS_HOSPITAL)]
        # Fallback 2: just primaryType==hospital
        if not filtered:
            filtered = [p for p in places if p.get('primaryType') == 'hospital']
        if not filtered:
            return jsonify({'name': None, 'km': None})
        places = filtered
    ...
    if place_type == 'supermarket':
        ...
        if major:
            places = major
        # else fall through to whatever Google returned (minor stores etc.)
