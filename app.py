...
def api_places_nearby():
    ...
    if place_type == 'hospital':
        ...
        # A place is considered a hospital if its NAME contains 'hospital' OR
        # its NAME contains a hospital-system word. We deliberately don't include
        # 'medical centre' because that's overwhelmingly a GP clinic.
        REQUIRED_NAME_KEYWORDS = [
            'hospital', 'health service', 'health network',
            # Major Victorian/NSW/Queensland hospital networks — these are real
            # hospitals even though the name doesn't contain "hospital".
            ...
            'royal childen', 'box hill', 'epworth', 'cabrini', 'peter maccallum'
        ]
        # Exclude these even if name matches above
        ...
        # Log what Google returned so we can debug bad results in production
        try:
            sample = [{'name': ((p.get('displayName') or {}).get('text')) or '',
                       'primaryType': p.get('primaryType', ''),
                       'types': p.get('types') or []}
                      for p in places[:5]]
            print('[/api/places-nearby hospital] returned', len(places),
        ...
        for p in places:
            ...
            if (primary == 'hospital'
                ...
                filtered.append(p)
        # Fallback: drop primaryType requirement, but still need name match
        if not filtered:
            for p in places:
                nm = ((p.get('displayName') or {}).get('text')) or ''
                if name_required(nm) and not name_excluded(nm):
                    filtered.append(p)
        if not filtered:
            return jsonify({'name': None, 'km': None})
        places = filtered
