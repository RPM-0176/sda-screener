...
def admin(section='activity'):
    ...
    for e in db.execute("SELECT e.*,u.full_name FROM events e JOIN users u ON e.user_id=u.id WHERE e.event_type='property_view' ORDER BY e.created_at DESC LIMIT 200").fetchall():
        ...
        try:
            ...
            row['addr'] = d.get('addr','n/a')
            row['state'] = d.get('state','').upper()
            row['sa4'] = d.get('sa4','n/a')
            row['price'] = '$'+str(int(d['price'])) if d.get('price') else 'n/a'
        except:
            row['addr'] = row['state'] = row['sa4'] = row['price'] = 'n/a'
...
def admin_stats():
    ...
    """).fetchall()
    events = db.execute("""
        SELECT e.*,u.full_name,u.username FROM events e
        JOIN users u ON e.user_id=u.id ORDER BY e.created_at DESC LIMIT 200
    """).fetchall()
    db.close()
    return jsonify({
        'stats':{'total_users':total_users,'total_sessions':total_sessions,'total_events':total_events},
...
def add_user_form():
    ...
    try:
        ...
        db.commit()
        db.close()
        return redirect('/admin/users?msg='+full_name+'+added+successfully&msg_type=ok')
...
def change_password(uid):
    ...
    if request.content_type and 'json' in request.content_type:
        return jsonify({'ok': True})
    return redirect('/admin/users?msg=Password+updated+successfully&msg_type=ok')
...
def toggle_user(uid):
    ...
    if request.content_type and 'json' in request.content_type:
        return jsonify({'ok': True})
    return redirect('/admin/users?msg=User+access+updated&msg_type=ok')
